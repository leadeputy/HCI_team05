...
hand_image_open = pygame.transform.scale(hand_image_open, (80, 80))
hand_image_closed = pygame.image.load(hand_image_closed_path)
hand_image_closed = pygame.transform.scale(hand_image_closed, (80, 80))
...
trashbin_images = [pygame.transform.scale(pygame.image.load(os.path.join(current_path, f'game1/trashbin/{i}')), (300, 150)) for i in trashbin_list]

spacing = 300  # 쓰레기통 사이의 간격
trashbin_width = trashbin_images[0].get_width()
# 초기 x 좌표 설정
initial_x = (screen_width)/32
...
class Item:
    ...
    def update(self, new_x, new_y, hold):
        if hold == self.item_type:
            self.x = new_x - self.image.get_width() // 2  # 중앙으로 조정
            self.y = new_y - self.image.get_height() // 2  # 중앙으로 조정
...
def handle_game2_loop():
    ...
    # 청소도구 이미지 로드
    cleaning_list = ["broomstick.png", "can.png", "paper.png"]
    cleaning_images = [pygame.transform.scale(pygame.image.load(os.path.join(current_path, f'game2/cleaning/{i}')), (120, 150)) for i in cleaning_list]
    broomstick_image, can_image, paper_image = cleaning_images
    
    #쓰레기 이미지 로드
    item_list = ["dust.png", "can2.png", "paper2.png"]
    item_images = [pygame.transform.scale(pygame.image.load(os.path.join(current_path, f'game2/item/{i}')), (120, 120)) for i in item_list]
    dust_image, can2_image, paper2_image = item_images
            ...
            if event.type == pygame.QUIT:
                ...
                for item in items:
                    if item.rect.collidepoint(mouse_pos):
                        hold = item.item_type
                        item.update(mouse_pos[0], mouse_pos[1], hold)
            elif event.type == pygame.MOUSEBUTTONUP:
                hold = 0
            elif event.type == pygame.MOUSEMOTION:
                if hold != 0:
                    for item in items:
                        if hold == item.item_type:
                            item.update(event.pos[0], event.pos[1], hold)
