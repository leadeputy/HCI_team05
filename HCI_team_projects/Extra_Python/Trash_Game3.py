...
# 버튼 만들기
buttons = [
    pygame.Rect(700, 490, 480, 100),  # "GAME START" on the first screen
    pygame.Rect(450, 535, 380, 100),  # "GAME START" on the second screen
    pygame.Rect(450, 525, 380, 100),  # "NEXT" on the third screen
    pygame.Rect(450, 525, 380, 100)   # "START" on the fourth screen
]

# 게임1 이후 화면의 버튼
next_buttons = [
    pygame.Rect(450, 525, 380, 100),  # "NEXT" on the NextGame.png screen
    pygame.Rect(450, 525, 380, 100),  # "NEXT" on the image5 screen
    pygame.Rect(450, 525, 380, 100)   # "START" on the image6 screen
]
...
class Trash:
    ...
    def follow_hand(self, index_finger_tip, thumb_tip):
        self.x = int((index_finger_tip[0] + thumb_tip[0]) / 2 - self.image.get_width() // 2)
        self.y = int((index_finger_tip[1] + thumb_tip[1]) / 2 - self.image.get_height() // 2)
    ...
    while current_screen < len(intro_images):
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                # 손가락 위치를 시각적으로 표시
                pygame.draw.circle(game_screen, (0, 255, 0), (index_x, index_y), 10)  # Draw a green circle at the index finger tip
                pygame.draw.circle(game_screen, (0, 0, 255), (thumb_x, thumb_y), 10)  # Draw a blue circle at the thumb tip

                # 현재 시간이 이전 제스처 인식 시간에서 1.5초 이상 지났는지 확인
                current_time = time.time()
                if current_time - last_gesture_time > 1.5:
                    # 버튼 영역을 감지
                    if buttons[current_screen].collidepoint(index_x, index_y):
                        # 손가락 끝과 엄지 손가락 끝의 거리 계산
                        distance = ((index_x - thumb_x) ** 2 + (index_y - thumb_y) ** 2) ** 0.5
                        if distance < 40:  # 손가락과 엄지 손가락이 가까이 있는 경우 (손을 쥐는 제스처)
                ...
                for trash in random_trash:
                    if holding_trash is None and trash.x < hand_center[0] < trash.x + trash.image.get_width() and trash.y < hand_center[1] < trash.y + trash.image.get_height():
                        ...
                        if distance < 40:  # 손가락과 엄지 손가락이 가까이 있는 경우 (손을 쥐는 제스처)
                            holding_trash = trash
                    if holding_trash:
                        holding_trash.follow_hand([index_x, index_y], [thumb_x, thumb_y])
    ...
    while current_screen < len(next_images):
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                # 현재 시간이 이전 제스처 인식 시간에서 3초 이상 지났는지 확인
                current_time = time.time()
                if current_time - last_gesture_time > 3:
                    # 버튼 영역을 감지
                    if next_buttons[current_screen].collidepoint(index_x, index_y):
                        # 손가락 끝과 엄지 손가락 끝의 거리 계산
                        distance = ((index_x - thumb_x) ** 2 + (index_y - thumb_y) ** 2) ** 0.5
                        if distance < 40:  # 손가락과 엄지 손가락이 가까이 있는 경우 (손을 쥐는 제스처)
                            print(f"Gesture detected: Moving to screen {current_screen + 1}")
